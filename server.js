...
function startRound(room) {
  ...
  room.answers = []; for (const player of room.players.values()) { player.answer = null; player.roundPoints = 0; player.usedPowerup = null; }
...
function reveal(room) {
  ...
  for (const player of room.players.values()) { if (!player.answer) continue; const key = normalize(player.answer); if (!groups.has(key)) groups.set(key, []); groups.get(key).push(player); }
  room.answers = [...groups.values()].flatMap(group => group.map(player => {
    ...
    if (size > 1) { player.streak += 1; player.bestStreak = Math.max(player.bestStreak, player.streak); points += Math.min(5, player.streak); } else player.streak = 0;
...
function create(ws, name, sessionId) {
  ...
  const room = { code: roomCode, hostId: null, phase: 'lobby', round: 0, totalRounds: 6, mode: '', modeLabel: '', prompt: '', options: [], chaos: '', deadline: null, answers: [], players: new Map(), timer: null };
...
function handle(ws, message) {
  ...
  if (message.type === 'answer' && room.phase === 'answering' && !player.answer) { const text = String(message.text || '').trim().slice(0, 90); if (!text) return; player.answer = text; if ([...room.players.values()].filter(item => item.connected).every(item => item.answer)) reveal(room); else broadcast(room); }
...
wss.on('connection', ws => { ws.on('message', data => { try { handle(ws, JSON.parse(data)); } catch { send(ws, { type: 'error', message: 'Something went wrong. Try again.' }); } }); ws.on('close', () => { const room = rooms.get(ws.roomCode); if (!room) return; const player = room.players.get(ws.playerId); if (player && player.ws === ws) { player.connected = false; broadcast(room); } }); });
